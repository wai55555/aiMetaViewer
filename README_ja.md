...
- **高度なリンク解析**:
  ...
  - **Civitai**: モデルのバージョン切り替えを検知し、バッジを即座にリフレッシュ。モデルファイル（.safetensors, .ckpt）とサンプル画像を正確に検出
...
- **一括スキャン＆ダウンローダー**:
  ...
  - **柔軟なフォルダ構成**: ID_ページタイトル、ページタイトル、ドメイン名、またはフラットな構造で自動整理
...
- **ダウンローダー設定**: 
  - 保存先のサブフォルダ構成（ID_ページタイトル、ページタイトル、ドメイン名、なし）を選択
...
└─ extension/          # Chrome拡張機能のソースコード
   ...
   ├── content.js      # Content Script (Badge管理)
    ├── scanner.js      # スキャナーのエントリーポイント＆メッセージング
    ├── scanner/        # スキャナーモジュール（ロジックとUIを分離）
    │  ├── core.js      # 画像収集・フィルタリングロジック
    │  ├── controller.js # 状態管理・イベントハンドリング
    │  ├── ui.js        # ダウンローダーモーダルUI
    │  ├── utils.js     # ヘルパー関数
    │  ├── progress-ui.js # スキャン進捗表示UI
    │  └── thumbnail-finder.js # サムネイル検出
    ├── parser.js       # メタデータ解析ロジック
    ├── adapters.js     # サイト別アダプター (Discord, Pixiv, Civitai 等)
    ├── ui.js           # 詳細ビューアUI
    ├── badge_controller.js # バッジの位置制御・ライフサイクル管理
